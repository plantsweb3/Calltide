...
function NavLink({ item, pathname, badges, onClose }: { item: NavItem; pathname: string; badges: NavBadges; onClose: () => void }) {
  ...
  return (
    <Link
      ...
      className="flex items-center gap-3 rounded-lg px-3 py-2 text-[13px] transition-all duration-150"
      style={{
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-2">
          ...
          <div className="space-y-0.5">
            {primaryNavItems.map((item) => (
              <NavLink key={item.href} item={item} pathname={pathname} badges={badges} onClose={onClose} />
            ))}
          </div>
        ...
        >
          <Link
            ...
            target="_blank"
            onClick={onClose}
            className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-[13px] transition-all duration-150"
            style={{ color: "var(--sidebar-text)" }}
            onMouseEnter={(e) => { e.currentTarget.style.background = "var(--sidebar-hover)"; e.currentTarget.style.color = "var(--sidebar-text-active)"; }}
