...
export const metadata = {
  title: "Política de Privacidad — Calltide",
  description: "Política de Privacidad de Calltide",
};

export default async function PrivacyEsPage() {
  let content = PRIVACY_ES;
  let version = "2.0";
      ...
      <header className="border-b" style={{ borderColor: "#E2E8F0" }}>
        <div className="mx-auto flex max-w-4xl items-center justify-between px-4 py-4">
          ...
          <div className="flex items-center gap-4 text-sm" style={{ color: "#475569" }}>
            ...
            <Link href="/legal/privacy" className="text-xs hover:underline" style={{ color: "#94A3B8" }}>EN</Link>
          </div>
        </div>
      </header>
      <main className="mx-auto max-w-4xl px-4 py-12">
        <div className="mb-8 rounded-lg border p-4" style={{ background: "#F8FAFC", borderColor: "#E2E8F0" }}>
          <p className="text-xs" style={{ color: "#94A3B8" }}>Versión {version} &middot; Vigente desde {new Date(effectiveDate).toLocaleDateString("es", { month: "long", day: "numeric", year: "numeric" })}</p>
        </div>
