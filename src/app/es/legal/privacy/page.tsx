...
export const metadata = { title: "Política de Privacidad — Calltide", description: "Política de Privacidad de Calltide" };
...
export default async function PrivacyEsPage() {
  ...
  try {
    const [doc] = await db.select().from(legalDocuments)
      .where(and(eq(legalDocuments.documentType, "privacy_policy"), eq(legalDocuments.isCurrentVersion, true))).limit(1);
  ...
  return (
    <main className="mx-auto max-w-4xl px-4 py-14" style={{ background: "#FBFBFC" }}>
      <div className="mb-2">
        <h1 className="text-3xl font-extrabold tracking-tight" style={{ color: "#1A1D24" }}>Política de Privacidad</h1>
      </div>
